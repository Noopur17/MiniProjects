...
orderApp.controller('homeCntrl', function($scope, $http) {
  ...
  $http.get("http://localhost:1000/orders").success(function(data) {
	  $scope.info = data;
  })
  
  $scope.delOrder = function(id) {
	  $http.get("http://localhost:1000/delorder?id="+id).success(function(data) {
		  $scope.info = data;
	  })
  }
...
orderApp.controller('newOrderCntrl', function($scope, $http, $location) {
	...
	$scope.addNewOrder = function() {
		$http.get("http://localhost:1000/neworder?desc="+$scope.desc+"&amt="+$scope.amt).success(function(data) {
			$scope.info = data;
